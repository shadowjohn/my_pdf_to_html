...
class HTMLConverter(PDFConverter[AnyIO]):
    ...
    def write_header(self) -> None:
        ...
        if self.codec:
            ...
            s = s+ "<style>";
            s = s+ "html,body{padding:0px;margin:0px;}";
            s = s+ "</style>";
        #self.write(s)
    ...
    def write_footer(self) -> None:
        ...
        self.PDF_INFO["TOTAL_PAGE"] = self.pageno
    ...
    def place_rect(
        self, color: str, borderwidth: int, x: float, y: float, w: float, h: float, dd: None
    ) -> None:
        ...
        d = {
            "kind": "DIV_OUTPUT",
            "page_num": self.PDF_INFO["CURRENT_PAGE"],
            "border": borderwidth,
            "color":color2,
            "x":x,
            "y":y,
            ...
            "h":h,
            "other":dd,
            "ZINDEX": self.PDF_INFO["ZINDEX"]
        }
        self.OUTPUT["OUTS"].append(d)
    ...
    def place_border(self, color: str, borderwidth: int, item: LTComponent) -> None:
        ...
        try:
            dd["fill"] = str(item.fill)
            #print(dir(item))
            #sys.exit()
        except:
            pass
        try:
            dd["stroke"] = str(item.stroke)
        except:
            pass
        try:
            dd["stroking_color"] = str(item.stroking_color)
        except:
            pass
        try:
            dd["linewidth"] = str(item.linewidth)
        except:
            pass
        try:
            dd["non_stroking_color"] = str(item.non_stroking_color)
        except:
            pass
           
        #print(dd)
        self.place_rect(color, borderwidth, item.x0, item.y1, item.width, item.height, dd)
        return
    ...
    def place_text(
        ...
        self.PDF_INFO["ZINDEX"] = self.PDF_INFO["ZINDEX"]
        self.OUTPUT["OUTS"].append(d)
        return
    ...
    def end_div(self, color: str) -> None:
        ...
        #self.write("</div>")
        #self.OUTPUT["OUTS"][-1]["font"]=self._font;
        for i in range(len(self.OUTPUT["OUTS"])-1,-1,-1):
                if self.OUTPUT["OUTS"][i]["kind"] == "TEXT_OUTPUT":
                    self.OUTPUT["OUTS"][i]["font"]=self._font;
                    break
        return
    ...
    def put_text(self, text: str, fontname: str, fontsize: float) -> None:
        ...
        if font != self._font:
            ...
            for i in range(len(self.OUTPUT["OUTS"])-1,-1,-1):
                if self.OUTPUT["OUTS"][i]["kind"] == "TEXT_OUTPUT":
                    self.OUTPUT["OUTS"][i]["font"] = font        
                    break
        self.write_text(text)
        for i in range(len(self.OUTPUT["OUTS"])-1,-1,-1):
                if self.OUTPUT["OUTS"][i]["kind"] == "TEXT_OUTPUT":
                    self.OUTPUT["OUTS"][i]["text"] = self.OUTPUT["OUTS"][i]["text"] + text
                    self.OUTPUT["OUTS"][i]["font-size"] = fontsize
                    break
        #self.OUTPUT["OUTS"][-1]["text"] = self.OUTPUT["OUTS"][-1]["text"] + text
        #self.OUTPUT["OUTS"][-1]["font-size"] = fontsize
        return
    ...
    def put_newline(self) -> None:
        ...
        for i in range(len(self.OUTPUT["OUTS"])-1,-1,-1):
                if self.OUTPUT["OUTS"][i]["kind"] == "TEXT_OUTPUT":
                    self.OUTPUT["OUTS"][i]["text"] = self.OUTPUT["OUTS"][i]["text"] + "\n"
                    break        
        return
    ...
    def receive_layout(self, ltpage: LTPage) -> None:
        def show_group(item: Union[LTTextGroup, TextGroupElement]) -> None:
            if isinstance(item, LTTextGroup):
                self.PDF_INFO["ZINDEX"] = self.PDF_INFO["ZINDEX"]+1
        ...
        def render(item: LTItem) -> None:
            ...
            if isinstance(item, LTPage):
                ...
                self.begin_div("figure", 1, item.x0, item.y0, item.width, item.height,item)
                for child in item:
                    render(child)
                self.end_div("figure")
            elif isinstance(item, LTImage):
                self.place_image(item, 1, item.x0, item.y0, item.width, item.height)
            else:
                if self.layoutmode == "exact":
                    ...
                    #    self.place_text(
                    #        "textbox", str(item.index + 1), item.x0, item.y0, 20
                    #    )
                    #    for child in item:
                    #        render(child)
                    #elif isinstance(item, LTChar):
                    ...
                    #    fontname = make_compat_str(item.fontname)
                    #    self.place_text(
                    #        "char", item.get_text(), item.x0, item.y0, item.size
                    #    )
                else:
                    if isinstance(item, LTTextLine):
                        for child in item:
                        ...
                        self.write_text(item.get_text())
            return
        self.PDF_INFO["CURRENT_PAGE"] = self.PDF_INFO["CURRENT_PAGE"]+1
        self.PDF_INFO["ZINDEX"] = 0
        #print("self.PDF_INFO[\"CURRENT_PAGE\"]: %d\n" %( self.PDF_INFO["CURRENT_PAGE"]))
        render(ltpage)
    ...
    def close(self) -> None:
        ...
        for k in self.OUTPUT.keys():
            jd = json.dumps(self.OUTPUT[k], ensure_ascii=False,indent=4, sort_keys=False)
            text = text+"var "+ k + "="+ jd+";\n";
        self.write(text)
        #print(text)
